...
for ntuple in input_file_list: #loop over files in a directory
	...
	#TrigMask_DC = 0b11101
	#flashermask = int(37778931862957161709553)
	flashermask = int(14)
	
	PrelimpassIndices = np.where((data.dcFlagged & PrelimMask) == PrelimMask)[0]
	#bg1 = np.where((data.dcFlagged & int(4)) == 0)[0]
	#bg2 = np.where((data.dcFlagged & int(8)) == 0)[0]
	#bg3 = np.where((data.dcFlagged & int(16)) == 0)[0]
	#bg = np.concatenate([bg1, bg2, bg3])
	#print(len(bg))
	#test = []
	#test2 = []
	#for b in bg:
	#	test.append(data.beta14[b])
	#	test2.append(data.itr[b])
	#print(test)
	#print(test2)
	#sys.exit()
	#print(data.dcFlagged & flashermask)	
	bg0 = np.where((data.dcFlagged & flashermask) == flashermask)[0]
	bg1 = np.intersect1d(bg0, PrelimpassIndices)
